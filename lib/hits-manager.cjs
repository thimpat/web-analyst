...
const {addIpInformationToMapper, buildIPIndexer, updateIPFile, buildTokenIndexer, updateTokenFile,
...
const {generateReferrerPopularityDataFromIndexer, saveReferrerIndexer} = require("./builders/tables/referers-chart.cjs");
 ...
 * @returns {string|null}
 */
const convertHitToString = function ({ip, acceptLanguage, userAgent, pathname, search, referer, cookieData})
{
    try
    {
        return getLineStats({
 ...
 * @param cookieData
 */
const updateChartFiles = function (cookieData = null)
{
    try
    {
        ...
        for (let i = 0; i < hitList.length; ++i)
        {
            ...
            if (cookieData) {
                addTokenInformationToMapper(cookieData.token, lineObj.ip);
            }
...
function updateBarChartsType(cookieData = null)
{
    try
    {
 ...
 * @returns {boolean}
 */
const registerHit = function ({ip, acceptLanguage, userAgent, pathname, search, referer, cookieData})
{
    try
    {
        ...
        updatePieChartsType();
        updateBarChartsType(cookieData);
