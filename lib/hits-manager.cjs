...
const {addToIpFile, initialiseIpMapFile, updateIPFile} = require("./builders/index/map-ips.cjs");
...
const {loadBrowserPopularityChartData, saveBrowserPopularityFile} = require("./builders/pies/browsers-charts.cjs");
const {initialiseBrowserMapFile, addToBrowserFile, updateBrowserFile} = require("./builders/index/map-browsers.cjs");
const {initialiseOSMapFile, updateOSFile, addToOSFile} = require("./builders/index/map-oses.cjs");
const {initialiseLanguageMapFile, updateLanguageFile, addToLanguageFile} = require("./builders/index/map-languages.cjs");
const {saveOsesPopularityFile, loadOsesPopularityChartData} = require("./builders/pies/oses-charts.cjs");
const {saveLanguagesPopularityFile, loadLanguagesPopularityChartData} = require("./builders/pies/languages-charts.cjs");
...
const updateMaps = function ()
{
    try
    {
        ...
        for (let i = 0; i < hitList.length; ++i)
        {
            ...
            addToOSFile(lineObj.userAgent);
            addToLanguageFile(lineObj.acceptLanguage);
        }
...
function updatePieChartsType()
{
    try
    {
        ...
        initialiseOSMapFile();
        initialiseLanguageMapFile();

        // Parse log entries to populate browser, os and language map files
        updateMaps();

        loadBrowserPopularityChartData();
        loadOsesPopularityChartData();
        loadLanguagesPopularityChartData();

        saveBrowserPopularityFile();
        saveOsesPopularityFile();
        saveLanguagesPopularityFile();
