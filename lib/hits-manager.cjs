const {getHitLogsPath, getLineStats,
    getHeaders, TABLE_SEPARATOR, buildStatsDir,
...
const {addToIpFile, buildIPIndexer, updateIPFile} = require("./builders/indexers/map-ips.cjs");
...
const {buildEndpointIndexer, addToEndpointIndexer, updateEndpointIndexer} = require("./builders/indexers/map-endpoints.cjs");
const {generateEndpointPopularityDataFromIndexer, saveEndpointIndexer} = require("./builders/tables/endpoints-chart.cjs");
const {generateMoneyChartData, saveMoneyFile, updateMoneyContent} = require("./builders/bars/money-chart.cjs");
const {SUB_DATA_DIR} = require("../constants.cjs");
...
const startLogEngine = function (server, namespace, {options = {}} = {})
{
    try
    {
        let outputDir = options.dir ? resolvePath(options.dir) : resolvePath(process.cwd());

        // Create directory for server
        buildStatsDir(server, namespace, {outputDir});
