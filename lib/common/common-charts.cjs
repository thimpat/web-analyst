 ...
 * @returns {boolean}
 */
const determineReturningVisitor = (ipInfo, infoType) => {
    try {
        if (!ipInfo) {
            return false;
        }
 ...
 *
 */
const updateVisitors = (currentDataByPeriod, ip, index, {info, infoType, cookieData = null} = {}) =>
{
    try
    {
        ++currentDataByPeriod.dataVisitors[index];

        const returningVisitor = determineReturningVisitor(info, infoType);
        if (returningVisitor) {
            return;
        }
