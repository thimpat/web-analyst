...
const updateMoney = (currentData, {lineObj, money = 0} = {}) =>
{
    try
    {
        const today = parseNow();
        const indexMonth = today.month;
        currentData.money[indexMonth] += money;
    }
    catch (e)
    {
        console.error({lid: 2669}, e.message);
 ...
 * @returns {null|any}
 */
const loadVisitorsChartData = (pathname) =>
{
    try
    {
        // Get labels
...
const loadMoneyChartData = (pathname) =>
{
    try
    {
        // Get labels
        ...
        if (pathname === CHART_DATA_FILES.MONEY_DATA_FILENAME)
        {
            labels = getYearLabels();
        }
        else
        {
            console.error({lid: 2353}, `Unsupported pathname`);
            return null;
        }

        const jsonPath = getChartPath(CHART_DATA_FILES.MONEY_DATA_FILENAME);
        ...
        {
            json.date = todayDate;
            json.money = new Array(labels.length).fill(0);
            writeFileSync(jsonPath, JSON.stringify(json, null, 2), {encoding: "utf8"});
        }
...
module.exports.loadMoneyChartData = loadMoneyChartData;
