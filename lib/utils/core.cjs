...
const {MEANINGFUL_LOG_FILES, SUB_DATA_DIR} = require("../../hybrid/cjs/wa-constants.cjs");
...
const buildWebContentDirectory = (outputDir) =>
{
    ...
    {
        console.error({lid: "WA2663"}, e.message);
    }
 ...
 * @returns {string}
 */
const getLineStats = function ({ip, acceptLanguage, userAgent, pathname, search, referer, cookieData = null})
{
    let strLine;
    try
    {
        const obj = getStructuredLine({
            ...
            pathname      : pathname || "",
            referer       : referer || "",
            token: cookieData ? cookieData.token : "",
        });
...
const getHeaders = function ()
{
    ...
    {
        console.error({lid: "WA2555"}, e.message);
    }
