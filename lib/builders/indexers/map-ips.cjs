 ...
 * @returns {boolean}
 */
function updateMapperList(info, dataEntryList, ip, {token = null, clientSideData = null}) {
    try {
        ...
        if (info)
        {
            ...
            if (token) {
                ...
                const lastVisitedPage = clientSideData.visitedPages.pop();

                dataEntryList[token].extra = dataEntryList[token].extra || {visitedPages: []};
                clientSideData.visitedPages = [...dataEntryList[token].extra.visitedPages, lastVisitedPage];
                dataEntryList[token].extra = clientSideData;
 ...
 * @returns {boolean}
 */
function addTokenInformationToMapper(token, ip, clientSideData)
{
    try
    {
        const tokenList = getRegisteredTokens();
        const infoToken = getRegisteredInfoToken(token);
        updateMapperList(infoToken, tokenList, ip, {token, clientSideData});
        return true;
    }
    catch (e)
    {
